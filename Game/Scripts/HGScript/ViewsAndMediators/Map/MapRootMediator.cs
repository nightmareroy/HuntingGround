...
//            selectedNodeList.Clear();
            ...
            {
                List<MapNavNode> autoGeneratedPath = mapRootView.Path<MapNavNode>(selectedNode, thisNode, mapRootView.newNodeCostCallback, selectedRoleInfo);
...
//            cost=dLandformDic[gameInfo.map_info.landform[selectedRoleInfo.pos_id]].cost;
            for (int i = 0; i < selectedRoleInfo.direction_param.Count; i++)
            {
                ...
                if (i == 0)
                {
                    cost += mapRootView.newNodeCostCallback(selectedNode, mapRootView.NodeAt<MapNavNode>(selectedRoleInfo.direction_param[i]), selectedRoleInfo);
                }
                else
                {
                    cost += mapRootView.newNodeCostCallback(mapRootView.NodeAt<MapNavNode>(selectedRoleInfo.direction_param[i-1]), mapRootView.NodeAt<MapNavNode>(selectedRoleInfo.direction_param[i]), selectedRoleInfo);
                }
