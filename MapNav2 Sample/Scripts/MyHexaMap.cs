...
public class MyHexaMap : MapNavHexa
{
	...
	private void UpdateMarked()
	{
		...
		{
			GameObject go = transform.FindChild("Tile " + selectedNode.idx.ToString()).gameObject;
		...
		{
			GameObject go = transform.FindChild("Tile " + altSelectedNode.idx.ToString()).gameObject;
		...
		if (int.TryParse(SampleGUI.Instance.selRadius, out radius))
		{
			if (radius > 0)
			{
				...
				if (nodes != null)
				{
					...
					{
						GameObject go = transform.FindChild("Tile " + nodes[i].idx.ToString()).gameObject;
